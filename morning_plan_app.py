...
"""
盘前 Plan 薄封装（P2）：morning_app + 组合/标的量化上下文 + auto_generate。
"""
import json
import os
import subprocess
...
def main():
    ...
    plan = {
        ...
        "needs_hermes": True,
        "instruction": (
            "必读 quant_bundle（含 market_regime + 持仓 per_stock 量化上下文）与 morning_output.json；"
            "确认 signal_auto_generate.total_signals>0；输出今日计划≤400字；"
            "推用户前可调 digest_app.py morning 生成②早计划短条。禁止 bash 取数。"
        ),
    }
